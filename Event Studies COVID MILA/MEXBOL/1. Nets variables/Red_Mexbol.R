...
#Tres eventos 11/03/2020, 23/03/2020 y 24/12/2020
...
par(mfrow=c(2,2), mar=c(1,1,1,1))
plot(mst[[468]], layout=layout.sphere, main="sphere",vertex.label=nodes2$id)
plot(mst[[468]], layout=layout.circle, main="circle",vertex.label=nodes2$id)
plot(mst[[468]], layout=layout.random, main="random",vertex.label=nodes2$id)
plot(mst[[468]], layout=layout.fruchterman.reingold, main="fruchterman.reingold",vertex.label=nodes2$id)
...
eigen_mat<-matrix(, nrow = 34, ncol = length(W))
...
write.csv(eigen_mat,'C:pruebaeigen_mexbol.csv', row.names = TRUE)
write.csv(centr,'C:pruebacent_mexbol.csv', row.names = TRUE)