...
use "C:finaldataset_SPBLPGPP.dta", clear

generate nombre_evento=cond(EVENT_DATE==td(11/03/2020),1, cond(EVENT_DATE==td(16/03/2020),2,3))
...
regress RET MARKET_RETURN V1 V2 if Company_id==5 & estimation_window==1 & nombre_evento==1
predict r if Company_id==5, residuals
...
arch RET MARKET_RETURN V1 V2 if Company_id==9 & estimation_window==1 & nombre_evento==1, arch(1/1) nolog
...
regress RET MARKET_RETURN V1 V2 if Company_id==11 & estimation_window==1 & nombre_evento==1
predict r if Company_id==11, residuals
...
arch RET MARKET_RETURN V1 V2 if Company_id==23 & estimation_window==1 & nombre_evento==1, arch(1/1)  nolog
...
regress RET MARKET_RETURN V1 V2 if Company_id==5 & estimation_window==1 & nombre_evento==2
predict r if Company_id==5, residuals
...
arch RET MARKET_RETURN V1 V2 if Company_id==9 & estimation_window==1 & nombre_evento==2, arch(1/1)  nolog
predict r if Company_id==9, residuals dynamic(.) 
...
arch RET MARKET_RETURN V1 V2 if Company_id==11 & estimation_window==1 & nombre_evento==2, arch(1/1)  nolog
...
regress RET MARKET_RETURN V1 V2 if Company_id==9 & estimation_window==1 & nombre_evento==3
predict r if Company_id==9, residuals 
...
regress RET MARKET_RETURN V1 V2 if Company_id==24 & estimation_window==1 & nombre_evento==3
predict r if Company_id==24, residuals 
...
arch RET MARKET_RETURN V1 V2 if Company_id==27 & estimation_window==1 & nombre_evento==3, arch(1/1)  nolog
predict r if Company_id==27, residuals dynamic(.) 
...
etable, estimates(Modelo1_3 Modelo1_19 Modelo1_21 Modelo1_26 Modelo1_29) column(dvlabel)                        ///
        ...
        export(PERU1.docx, replace) 
 
 etable, estimates(Modelo2_3 Modelo2_19 Modelo2_21 Modelo2_26 Modelo2_29) ///
 column(dvlabel)                        ///
        cstat(_r_b, nformat(%4.2f))                                   ///
        showstars showstarsnote                                       /// 
        stars(.10 "*" .05 "**" .01 "***", attach(_r_b))              ///
        ...
        titlestyles(font(Lucida Console, size(14) bold))              ///
        notestyles(font(Lucida Console, size(10) italic))             ///  
        export(PERU2.docx, replace) 
  
etable, estimates(Modelo3_3 Modelo3_19 Modelo3_21 Modelo3_26 Modelo3_29) column(dvlabel)                        ///
        cstat(_r_b, nformat(%4.2f))                                   ///
        showstars showstarsnote                                       /// 
        stars(.10 "*" .05 "**" .01 "***", attach(_r_b))              ///
        mstat(N,   nformat(%8.0fc) label("Observations"))             ///
...
export excel using "C:\Users\migue\iCloudDrive\Estudio de eventos articulo\STATA\Estudio_eventosSPBLPGPP.xlsx", sheet("eventoscompletos") firstrow(variables)

export excel using "C:Estudio_eventosSPBLPGPP.xlsx" if nombre_evento==1, sheet("OMS Declara COVID") firstrow(variables)

export excel using "C:Estudio_eventosSPBLPGPP.xlsx" if nombre_evento==2, sheet("Primer Confinamiento") firstrow(variables)

export excel using "C:Estudio_eventosSPBLPGPP.xlsx" if nombre_evento==3, sheet("Primer día Vacunación") firstrow(variables)
...
snapshot restore 1
outreg2 [Modelo*] using resultadosmodeloSPBLPGPP.xls, replace
...
export excel using "C:Estudio_eventosSPBLPGPP.xlsx", sheet("Estat_var_AR") firstrow(variables) 
