...
#Tres eventos 11/03/2020, 25/03/2020 y 17/02/2021
...
par(mfrow=c(2,2), mar=c(1,1,1,1))
plot(mst[[470]], layout=layout.sphere, main="sphere",vertex.label=nodes2$id)
plot(mst[[470]], layout=layout.circle, main="circle",vertex.label=nodes2$id)
plot(mst[[470]], layout=layout.random, main="random",vertex.label=nodes2$id)
plot(mst[[470]], layout=layout.fruchterman.reingold, main="fruchterman.reingold",vertex.label=nodes2$id)

par(mfrow=c(2,2), mar=c(1,1,1,1))
plot(mst[[705]], layout=layout.sphere, main="sphere",vertex.label=nodes2$id)
plot(mst[[705]], layout=layout.circle, main="circle",vertex.label=nodes2$id)
plot(mst[[705]], layout=layout.random, main="random",vertex.label=nodes2$id)
plot(mst[[705]], layout=layout.fruchterman.reingold, main="fruchterman.reingold",vertex.label=nodes2$id)
...
eigen_mat<-matrix(, nrow = 25, ncol = length(W))
...
EIGEN_cent<-eigen_centrality(mst[[705]], directed = FALSE, scale = TRUE,options = arpack_defaults)$vector
plot.igraph(mst[[705]],vertex.size=EIGEN_cent*30,layout=layout.fruchterman.reingold,
            vertex.label=nodes2$id,
            vertex.frame.color="gray", vertex.label.color="black", edge.color="gray",
            vertex.label.cex=0.6,
            edge.width=as.numeric(E(mst[[705]])$weight)*2.5,
            vertex.label.dist=1,
            frame='False',
            edge.lty=c("solid"),
            margin=-0.65,
...
write.csv(eigen_mat,'C:pruebaeigen_colcap.csv', row.names = TRUE)
write.csv(centr,'C:pruebacent_colcap.csv', row.names = TRUE)
