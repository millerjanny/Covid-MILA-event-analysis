...
use "C:finaldataset.dta", clear

generate nombre_evento=cond(EVENT_DATE==td(11/03/2020),1, cond(EVENT_DATE==td(25/03/2020),2,3))
...
arch RET MARKET_RETURN V1 V2 if Company_id==2 & estimation_window==1 & nombre_evento==1, arch(1/1) nolog
...
arch RET MARKET_RETURN V1 V2 if Company_id==2 & estimation_window==1 & nombre_evento==2, arch(1/1) nolog
predict r if Company_id==2, residuals dynamic(.) 
...
arch RET MARKET_RETURN V1 V2 if Company_id==24 & estimation_window==1 & nombre_evento==3, arch(1/1) archm archmlags(1) nolog difficult
predict r if Company_id==24, residuals dynamic(.) 
...
etable, estimates(Modelo1_7 Modelo1_8 Modelo1_9 Modelo1_17 Modelo1_18 Modelo1_21) column(dvlabel)                        ///
        ...
        export(Colcap1.docx, replace) 
 
 etable, estimates(Modelo2_7 Modelo2_8 Modelo2_9 Modelo2_17 Modelo2_18 Modelo2_19 Modelo2_20) column(dvlabel)                        ///
        cstat(_r_b, nformat(%4.2f))                                   ///
        showstars showstarsnote                                       /// 
        stars(.10 "*" .05 "**" .01 "***", attach(_r_b))              ///
        mstat(N,   nformat(%8.0fc) label("Observations"))             ///
        ...
        note("Data Source:Bloomberg")                             ///
        notestyles(font(Lucida Console, size(10) italic))             ///  
        export(Colcap2.docx, replace) 
  
etable, estimates(Modelo3_7 Modelo3_8 Modelo3_9 Modelo3_12 Modelo3_13 Modelo3_17 Modelo3_18) column(dvlabel)                        ///
        cstat(_r_b, nformat(%4.2f))                                   ///
        showstars showstarsnote                                       /// 
        stars(.10 "*" .05 "**" .01 "***", attach(_r_b))              ///
        mstat(N,   nformat(%8.0fc) label("Observations"))             ///
...
export excel using "C:Estudio_eventosCOLCAP.xlsx", sheet("eventoscompletos") firstrow(variables) 

export excel using "C:Estudio_eventosCOLCAP.xlsx" if nombre_evento==1, sheet("OMS Declara COVID") firstrow(variables) 

export excel using "C:Estudio_eventosCOLCAP.xlsx" if nombre_evento==2, sheet("Primer Confinamiento") firstrow(variables) 

export excel using "C:Estudio_eventosCOLCAP.xlsx" if nombre_evento==3, sheet("Primer día Vacunación") firstrow(variables) 
...
snapshot restore 1
outreg2 [Modelo*] using resultadosmodelocolcap.xls, replace
...
export excel using "C:Estudio_eventosCOLCAP.xlsx", sheet("Estat_var_AR") firstrow(variables) 
